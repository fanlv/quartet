...
- 用户、角色和认证策略保存到 `LOCAL_MEMORY` 下的持久配置目录。
...
6. **持久配置与登录会话分离**：用户、密码摘要、角色和权限属于持久配置；Cookie 和服务端会话属于可撤销运行状态。
...
3. 用户填写管理员用户名、显示名称、密码和确认密码。
4. 初始化成功后创建第一个用户并授予内置 `admin` 角色，同时建立登录会话。
5. 初始化只能成功一次；并发提交时只允许一个请求成功，其他请求得到明确的冲突错误。
6. 初始化标记与用户数量分开判断。系统曾初始化后，即使用户文件损坏、被误删或没有可用管理员，也只能进入恢复状态，不能重新开放匿名初始化。
...
- 登录会话没有时间过期限制，服务重启后继续有效。
- 服务端只保存会话标识的摘要；Cookie 原值不写入文件日志、业务响应或可读前端存储。
- 每次请求都校验会话、用户状态和用户当前角色，不把权限固化在 Cookie 中。
- 退出登录删除当前服务端会话并清除 Cookie。用户停用、删除、管理员重置密码时撤销该用户全部会话。
- 会话不存在或已被撤销返回 `401`；已登录但权限不足返回 `403`，并返回完整、可定位的错误信息。Web 和 iOS 收到 `401` 后都应返回登录页面。
...
`quartet/config/auth/` 保存持久配置；`var/` 只保存运行状态。

### 7.2 持久化语义

- 用户和角色变更以原子方式落盘，写入成功后即对当前实例生效。
- 启动时完整校验认证配置。存在重复用户名、未知角色、未知权限、缺失管理员或不可解析文件时进入恢复状态，不静默修复或跳过。
- 外部修改认证文件后，需要显式重新加载或重启后端才生效；加载失败时保留上一份有效内存快照并展示完整错误。
- 恢复旧版用户配置可能还原旧密码摘要、账号状态或权限。恢复后应主动撤销全部会话，并要求用户重新登录。
- 密码摘要属于敏感配置，Memory 的存储位置和备份必须受到妥善保护。
...
| 初始化管理员 | `POST /api/v1/auth/init` | 待初始化 |
...
**用户故事：** 作为实例维护者，我希望用户与权限配置持久保存，以便备份和恢复。

**验收标准：**

- [ ] 初始化和后续用户、角色变更只修改 `$LOCAL_MEMORY/quartet/config/auth/` 下的持久配置。
- [ ] 登录、刷新页面和退出不会修改持久配置。
- [ ] Cookie、会话标识和 CSRF 凭证不会进入持久配置。
- [ ] 从备份恢复有效配置并重启后，用户与角色恢复，但旧会话不可复用。
...
9. 完成初始化、登录、权限矩阵、Cookie/SSE、配置恢复和多客户端回归验收。
