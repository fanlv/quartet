...
当前 E2E（见 `docs/feature-2026-06-03-web-e2e.md`）采用「真实链路 + 替换模型输出」模型：建 Job、发消息、SSE、历史全走真实后端，只把「产出模型输出」一环换成确定性回放，并为故障链路（发送失败 / SSE 断开 / resume 过期 / snapshot 重拉失败）在服务层内置一次性故障注入。
...
- 测试数据全部经真实业务接口创建：建 Job 走 `/job/create`，发消息走 `/job/message`，历史 / 侧边栏多 Job 经真实建 Job + 发消息预置。
- 断言对象调整为「真实模型流程下稳定可观察的结构信号」：消息节点出现、assistant 流式增量完成、工具块进终态、运行态 loading→idle、错误完整展示、列表排序 / 选中 / Rename / Delete / Pin 等纯前端行为。**不再断言固定自然语言文本片段**（真实模型输出不确定）。
...
| E2E 模式 ACP 拒绝（创建 / 发送） | 删除（依赖 E2E 模式）；ACP 在生产模式正常可用，无需 E2E 模式拒绝 |
...
- **断言稳定性**：真实模型输出不确定，所有断言改为结构 / 状态信号，禁止比对固定自然语言；工具用例断言「进入终态」而非具体内容。
