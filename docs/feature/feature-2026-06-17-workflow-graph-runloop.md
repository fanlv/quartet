...
**不做**：特殊汇聚策略（race/vote/N 选 K，只支持等全部入边解析）；循环内变量按轮隔离（确认不隔离，见 §3）；输出变量的对象、数组等独立类型（统一按字符串处理，但字符串可多行）。
...
- Prompt/评估没有控制文件，使用模型原始输出中的具名多行块协议。每个声明变量对应一个起止标记，块内内容按原文保存，可为空或包含换行、引号、等号，无需 JSON 或 Base64 转义；同名输出多次时取最后一个。Graph 在 prompt 后追加固定协议说明。
- 多行块的开始标记允许与前一段模型文本粘连，以兼容相邻消息合并；结束标记必须独占一行并与开始变量名一致。嵌套块、缺失或错配结束标记均判结点失败并显示完整错误。
- 旧的单行输出格式继续作为读取兼容入口。变量名仍须符合 `[A-Za-z_][A-Za-z0-9_]*`；`quartet_set` 的值由控制文件 base64 解码后原样保留。
...
- 声明了输出变量的节点必须产出所有已声明变量；缺任一已声明变量、变量名非法、多行块嵌套或边界不完整均判结点失败全量报错。Shell 错误含 stdout/stderr/exit code/控制文件内容或解析错误；Prompt/评估错误含模型原始输出与解析错误详情。
...
- 配了输出变量名时，Shell 具名变量写控制文件解析值，Prompt/评估具名变量从多行输出块解析；配了 `_last_assistant_msg` 别名时，别名变量由调度器自动写入本结点原始最终输出。下游 `_last_assistant_msg`、别名变量与具名输出变量皆可读，值来源不同。
...
8. **变量基础能力**（已完成，依赖：4、5）：初始变量、输出变量声明、`_last_assistant_msg` 别名变量、Shell 控制文件解析、Prompt/评估原生多行输出解析、实例可见快照、执行时替换、单写者串行落库、写同名变量保存校验、保留变量名保存校验、字符串比较选项处理。完成判定：变量声明、解析、替换、快照继承和冲突校验按 §1/§3 语义运行。（进度：Prompt/评估输出变量支持原文多行值，旧单行格式继续可读；DAG 调度器已将变量解析、快照继承、分支、循环与续跑接入统一的单写者持久化链路。）
...
| Prompt/评估输出协议：多行原文、空值、引号与等号、同名输出取最后；旧单行格式兼容；配置中重复声明保存失败；缺已声明变量、变量名非法、块嵌套或边界不完整时结点失败并显示完整错误（含模型原始输出） | 单测 + 手工 ✅ |
...
- **运行前保存模板修改（Web/iOS）**：从已保存 workflow 上修改配置后运行，会先以打开时的版本为条件保存模板；保存成功后才启动，保存失败或发生版本冲突时不启动。再次选择该工作流时会恢复最近保存的配置。
