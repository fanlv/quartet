...
**GraphRun 与 Job 关系**：GraphRun 是 Graph 类型 Job 的一次执行明细与快照；用户侧仍以 Job 作为运行列表、Chat 运行区、历史入口与运行控制的承载对象。每次启动 GraphWorkflow 创建一个 Graph 类型 Job 并绑定一个 GraphRun，统一出现在现有运行/历史视图；启动/停止/暂停/步骤后停止/续跑/查看状态与历史都落到绑定 GraphRun。删除 workflow 不影响已绑定 Job 历史的 GraphRun。
...
8. 历史 run 按各实例执行时版本展示图/进度/日志/错误，后续编辑都有版本记录可回放；可显式删除非在飞历史 run/Job，删除时级联清理其全部持久化产物。
...
- `QUARTET_OUTPUT` **行首锚定**：trim 行首空白后仅匹配「行首紧跟 `QUARTET_OUTPUT:`」的行，前缀有非空白字符不算。
- 变量名 `[A-Za-z_][A-Za-z0-9_]*`；`QUARTET_OUTPUT` 按每行首个 `=` 切分，变量名 trim，值原样保留（可空、可含 `=`、不 trim）；只支持单行标量。`quartet_set` 的值由控制文件 base64 解码后原样保留。
...
- **`_last_assistant_msg`**（内置）：默认写结点原始最终输出——Prompt/评估为模型原始输出，Shell 为 stdout 全量（不含 stderr，无输出为空串）。取值以 §3 实例快照血缘为准（非全局「时序最近」）：单入边继承上游快照值；多入边 join 按已激活入边「上游结点 ID 升序的末位」取值（不依赖到达顺序、被剪枝上游不参与），保证多次运行与崩溃恢复/续跑取值一致可重放；需稳定引用某上游应配明确输出变量名。条件中整体按字符串参与（多行/大文本不切分不截断），支持所有字符串比较操作符和比较选项。
...
- **步骤后停止**（GraphLoop）：「步骤」= 当前 ready 批次。每次从 ready 队列取可调度实例时生成批次 ID，持久化批次 ID、成员、成员状态（未启动/运行中/终态）；触发后冻结当前批次、进入「步骤停止中」、不再生成后续批次，已在飞的跑到终态、同批次未启动成员按并发额度继续启动；本批次全部成员终态后进入「已步骤停止」，未进入本批次的实例保持待定。步骤停止中崩溃恢复先按持久化批次补完该批次再保持「已步骤停止」；续跑清除批次冻结状态、从已解析入边重推队列。循环内触发停在当前批次边界、不强制等完整轮次。
...
- Agent 类结点声明会话策略——新建，或继承上游（仅允许唯一入边）。
- 并行分叉时每条分支复制上游上下文后**新建独立会话**（不复用 session ID 避免污染）；多入边 Agent 视为 join、必须新建（配为继承则保存报错）；约束「每条 start 链路首个可执行 Agent 必须新建会话」。运行时不依赖 roundMode。
...
- **运行配置与版本化编辑**：GraphRun 启动持久化基线快照（nodes/edges/初始变量/disabledVars/布局/workspace 与 workdir〔及 sandbox〕/运行配置〔并发度、结点级与 job 级超时、循环最大次数兜底、实例总数与快照体积上限〕/会话策略/被引用 Agent 与模型配置内容）。运行中保存编辑时，不直接覆盖基线快照，而是追加一个 GraphRun 图版本，版本内容包含完整 nodes/edges/布局、结点配置、会话策略，以及被引用 Agent/模型配置的内容快照。
  ...
  - **历史回放**：每个实例状态记录执行时的图版本号；历史 run 可按版本展示当时布局和配置。删除 workflow 不删既有 run 的基线快照、图版本和运行记录，除非用户显式删除该 run。
...
- **崩溃恢复**：重启从盘上状态重建就绪队列，运行中实例与可重置终态按重置规则重跑，不可重置终态不重复执行。
...
| 步骤停止中 | 触发步骤后停止，当前冻结批次继续 | 否 | 等待持久化批次全部成员终态；崩溃恢复先补完该批次。 |
...
- API 错误不得只给摘要；前端可折叠长 stdout/stderr/model 输出，但必须能展开看完整内容。历史 run 保留当时错误详情。
...
- **`services/graph`**：图合法性校验、条件表达式解析求值、变量快照合并、调度执行、循环驱动、resume/recover、停止控制。
...
**状态：已完成**。后端运行日志已覆盖 GraphRun 生命周期、调度、并发、结点执行、边解析、join、循环、变量写入、会话、运行控制、版本更新、超时与失败；SSE 事件流已覆盖运行态与历史回放所需的实例状态、边状态、变量、循环、进度、日志与错误。Prompt/评估结点执行真实 Agent 时，Graph 现在会把 Agent 消息、思考、工具调用参数/结果/结束状态与 token usage 追加为 GraphRun 事件并通过 SSE 续传；Graph 类型 Job 的 Agent 结点用量也已接入 `usagestats`，统计页可看到 token、工具调用次数与工具耗时。
...
4. **图合法性校验**（已完成，依赖：1、3）：按 §1 全量校验 start/end、纯控制图、控制结点配置、不可达结点、入/出边、非法环、If-Else 端口、循环子图边界、条件表达式语法、变量写冲突、初始变量与输出变量同名覆盖、保留变量名、多入边 Agent 禁继承、评估结点强制 ≥1 输出变量且禁分支出边，错误全量返回带定位。完成判定：非法图保存/导入时一次返回全部定位错误，合法图可保存。（进度：`services/graph` 的 `validateConfig` 全量校验 + 条件表达式 tokenizer/递归下降 parser 已实现，仅做保存期静态语法校验〔运行时变量存在性与求值留给执行引擎〕；附 `validate_test.go`/`condition_test.go` 共 32 个单测全绿。）
5. **线性图调度闭环**（已完成，依赖：1、3）：实现就绪队列、入边计数、边三态、拓扑串行执行，先跑通 start → Shell/Prompt → end（可用最小内存 run 驱动，完整快照在步骤 6 接入）。完成判定：最小线性 Graph 类型 Job 可启动、执行、结束并得到成功/失败状态。（进度：`services/graph` 已实现第一阶段线性运行器，限定单 start、单 end、每个中间结点单入单出，支持 Shell/Prompt/评估结点顺序执行；GraphRun 元数据、实例状态、边状态、变量快照、progress、resume 与事件日志落到 `GraphRunRepo`，并通过窄接口同步绑定 Job 的 GraphRunID 与运行状态；新增 `/api/v1/graph/run/start`、`/api/v1/graph/run/:runId`、`/api/v1/graph/run/list`。该阶段主动拒绝分支、join、循环、并发等非线性图，后续步骤继续扩展。**步骤 9/10/11 落地后，线性运行器已被 `services/graph/scheduler.go` 的 DAG 就绪队列调度器取代**，分支/join/并发已支持；**步骤 13 落地后调度器进一步支持循环容器与嵌套循环子图，已不再拒绝任何合法图。**）
...
| Prompt/评估 `QUARTET_OUTPUT` 协议：行首锚定、变量名字符集、首个 `=` 切分、空串、值含 `=`、同名多行取最后、单行限制；配置中重复声明保存失败；模型输出未声明变量/缺已声明变量/变量名非法/值跨行时结点失败完整报错（含模型原始输出） | 单测 ✅ |
...
| 嵌套循环：按父容器 ID 分层、实例键唯一可恢复、嵌套续跑/崩溃恢复正确重建 | 单测 ⚠️（`TestNestedLoops` 已覆盖嵌套分层与实例键唯一执行；缺一条专门验证「嵌套循环续跑/崩溃恢复后正确重建迭代上下文」的 resume/recover 单测——现有 resume/recover 单测均为非嵌套循环场景） |
...
| 功能 | 测试方式 |
| --- | --- |
| 会话沿边继承/新建/并行分叉复制上下文后新建独立会话/多入边强制新建；多入边配继承时保存失败 | 单测 ✅ |
...
| 崩溃恢复基于实例/边状态/变量快照/会话血缘/循环上下文重建就绪队列，成功/跳过不重跑、失败/中断重置后重跑 | 单测 ✅ |
...
| 删除历史 GraphRun/Graph 类型 Job：仅允许删除非在飞 run，级联原子清理快照/图版本/实例·边状态/变量快照/会话血缘/事件日志，无残留且不影响其它 run；删除在飞 run 被拒绝并报错 | 单测 + 手工 ✅ |
...
| graph 工作流 CRUD/运行/硬停止/优雅停止/续跑/查看状态/历史/删除历史，入口与 Loop 并列 | E2E + 手工 ⚠️（CRUD/启动/运行至完成/历史回放/运行中编辑已有 `graph-canvas.spec.ts` E2E；硬停止/暂停/续跑/删除历史的后端接口已实现，但尚无 E2E 用例直接驱动这些控制动作，列为手工验收） |
| graph 接口接入统一路由与鉴权，错误完整返回前端 | E2E + 手工 ✅ |
| SSE 按实例键上报，刷新页面后恢复当前运行状态与历史日志 | E2E + 手工 ⚠️（SSE 接口与事件回放已实现并在运行查看链路使用；缺一条专门「运行中刷新页面恢复画布/日志/progress」的 E2E 用例，目前以手工验收） |
...
| 错误展示规格：API 返回完整 stdout/stderr/model 原始输出与定位字段（不只给摘要）；前端可折叠但必须能展开看完整内容；历史 run 保留错误详情 | E2E + 手工 ✅ |
...
| 步骤后停止=当前 ready 批次边界：批次 ID、成员与成员状态持久化；触发后进入「步骤停止中」，本批次完成后进入「已步骤停止」，不调度后续批次；崩溃恢复先完成被冻结批次，续跑清除冻结状态并从持久化状态继续 | E2E + 手工 ⚠️（后端批次冻结/恢复已实现且有 Go 单测；缺前端 E2E，列为手工） |
