...
- **缺少未保存改动确认（前端）**：返回、新建、切换到其它工作流、取消运行版本编辑等「离开当前文档」的操作此前都不检查未保存改动，复杂画布编辑可能被直接覆盖且不可恢复。修复后：这些入口在存在未保存改动时统一先行确认。
- **运行查看缺少边状态（前端）**：运行回放此前只展示结点状态，未消费运行记录里的边状态，分支执行路径（激活/剪枝/已完成边）不可见，而画布与 Chat 页的 GraphLoop 都已支持边状态显示。修复后：Graph 工作流页的运行查看接入边状态叠加，与 Chat 页运行视图一致。
