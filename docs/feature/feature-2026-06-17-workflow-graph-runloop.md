...
**2026-06-24 复核修正**：运行控制与编辑入口统一以 Job 绑定 GraphRun 为契约：前端深链使用 `?graphEditJob=<jobId>`，保存运行版本调用 `PUT /api/v1/job/:jobId/graph-run/version`，暂停/步骤后停止/硬停止/续跑分别调用 `/pause`、`/step-stop`、`/stop`、`/resume`。运行版本编辑不是「仅节点配置」模式：前端允许修复未冻结节点和边，新增循环容器时同步生成循环入口/出口标记；已成功/跳过/运行中节点的执行配置仍按后端冻结规则置灰并由后端兜底校验。调度器控制队列满时控制动作必须返回明确失败，不允许把未投递信号伪装成成功。Graph workflow 普通保存携带打开时捕获的 `updatedAt` 做乐观锁，不受列表刷新影响；不匹配返回 409，避免多标签静默覆盖。读取、删除或更新 workflow 时，仅真实不存在/已删除映射为 not found，文件读取或 JSON 解析失败按原始错误返回。前端所有 Graph 错误展示先保留响应原文，再尝试解析 JSON；非 JSON 错误不再退化成纯 HTTP 状态。未保存保护覆盖应用内离开、删除当前 workflow 与浏览器刷新/关闭。普通 Run Config 面板暴露实例数量上限与快照字节上限，数值输入只接受整数。Graph 状态/错误区通过 `aria-live` 通知异步校验与错误更新，关键 JSON/变量输入提供可访问名称。再次复核后补齐：画布载入既有配置后重新同步后续新增元素的 ID 分配，普通新增节点和连线也做碰撞检测；Chat 内嵌 GraphLoop 编辑器与主画布保持一致，支持把新增节点落入 loop 容器、删除 loop 时级联删除其子图、Inspector 的条件变量建议使用当前编辑中的图结构；带 workflowId 的运行请求如果来源 workflow 不存在、已删除或损坏，必须返回完整错误，不再静默降级为无来源的临时运行；GraphRun 状态查询只把真实不存在映射为 not found，其它读取或解析错误完整返回；workflow 保存时统一工作流记录与配置内的 workspace 归属。启动运行时必须先按 workflow/config 解析最终工作区与工作目录，再创建绑定 Job，避免只传 workflowId 时落到默认工作区；显式传入既有 Job 发起运行时，如果绑定 GraphRun 失败，必须清理本次创建的运行产物但不删除调用方拥有的 Job。删除 workflow 的确认动作必须防重复提交；Graph 表单控件必须提供稳定可访问名称；列表日期按当前语言地区显示并在跨年时显示年份；Inspector 键盘焦点必须清晰可见，动画只绑定明确属性。
...
- **Graph 状态与表单可访问性不足（前端）**：异步错误/校验结果此前不会主动通知读屏，JSON 与变量输入缺少明确可访问名称。修复后：状态区使用 live region，关键输入补充可访问名称。
...
- **GraphRun 元数据读取错误被伪装成不存在（后端）**：状态查询此前把读取 run 元数据的所有错误都映射为 not found。修复后：只把真实不存在映射为 not found，JSON 损坏、非法 ID 或底层读取失败按完整错误返回。
- **workflow 归属 workspace 双来源不一致（后端）**：workflow 记录和配置内部都携带 workspace 归属，此前 create/update 可保存出两者不一致的数据。修复后：服务层保存时统一归一，运行、调度和列表归属读取同一语义。
