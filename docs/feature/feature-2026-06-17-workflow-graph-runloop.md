...
- **对齐 Loop 体验**：Graph 配置覆盖 Loop 全部配置管理能力；Chat 页用独立 GraphLoop 组件承载运行展示与「编辑/步骤后停止」控制。
...
- **Chat 页兼容**：运行区按 job 类型选择旧 Loop 组件或 GraphLoop。GraphLoop 展示整体完成度与运行中/成功/失败/跳过/中断结点，并尽量在图视角标出当前结点、已走分支、循环轮次与 join 等待；无法展示完整画布时至少以列表/路径摘要表达当前位置。旧 Loop 任务仍用旧组件与旧语义。主画布和 Chat 内嵌运行版本编辑共用结构编辑规则（新增节点、loop 入口/出口生成、删除级联、控制节点可删除性），避免两处图结构行为分叉。
- **运行控制**：GraphLoop 保留「编辑/步骤后停止」入口。步骤后停止冻结并持久化当前 ready 批次、进入「步骤停止中」、批次全部成员终态后进入「已步骤停止」；运行中编辑保存为 GraphRun 图版本，已完成/运行中/当前冻结批次实例不受影响，后续未开始实例按最新有效版本执行。Chat 内嵌编辑必须具备与主画布一致的结构化校验错误定位；取消、切换 Job 或退出编辑前如存在未保存改动，必须先确认，用户取消时保留草稿。若内嵌编辑不保存全局变量与运行配置，这些全局配置控件必须禁用，不得展示可编辑但不生效的状态。
...
21. **Chat 页 GraphLoop 组件**（部分完成，依赖：16、17、18）：按 job 类型选择旧 Loop 组件或 GraphLoop；GraphLoop 展示 progress、当前位置、已走分支、循环轮次、join 等待，支持编辑/步骤后停止，不改旧 Loop 任务语义。完成判定：Chat 页能正确区分旧 Loop Job 与 Graph 类型 Job，GraphLoop 控制只影响绑定 GraphRun。（进度：Chat 页 hook 已识别 `mode=graph` 与绑定 `graphRunId`，旧 Loop 仍走 `LoopProgress`/`LoopSessionSidebar`；新增独立 `GraphLoopProgress` 组件，加载绑定 Job 的 GraphRun 状态并订阅事件，展示总进度、运行中实例、已激活/剪枝边、join 等待、循环轮次、最近事件与完整错误；支持编辑、步骤后停止、取消待停止、硬停止与续跑，所有控制动作只调用绑定 Job 的 GraphRun 后端接口；Chat 输入在 Graph Job 中禁用，避免混入交互消息；历史/侧边栏/Chat 下拉列表已区分 graph job 图标。GraphLoop 内嵌编辑使用当前有效版本快照，允许修复未冻结节点和边，保存调用 `PUT /api/v1/job/:jobId/graph-run/version`。剩余缺口：Chat 页 graph job 分流、GraphLoop 控制和步骤后停止等前端链路缺专项 E2E/组件测试。）
...
| Chat 页按 job 类型选择旧 Loop 组件或 GraphLoop；GraphLoop 显示总进度/结点状态/当前位置/已走分支/循环轮次/join 等待，支持编辑/步骤后停止，旧 Loop 语义不变 | E2E + 手工 ⚠️（`JobChat` 已按 `isGraph` 分流渲染 `GraphLoopProgress` 并实现编辑/步骤后停止/续跑控制；缺 Chat 页 graph job 的 E2E/组件测试，目前以手工验收） |
...
- **GraphRun 控制与运行编辑入口未闭环（前端）**：后端已有步骤后停止与运行版本保存接口，前端此前缺少步骤后停止按钮，且运行版本编辑被整体结构只读锁限制。修复后：GraphLoop 暴露步骤后停止，运行版本编辑允许修复未冻结结构，保存走绑定 Job 的版本接口。
...
**E2E 覆盖**：上述修复新增一组端到端用例（驱动真实后端，纯 Shell / 单 If-Else 图），覆盖——JSON 视图禁用全局操作并在切回画布时确认未应用草稿；运行启动期间运行按钮禁用且单次启动只产生一个 Graph Job；无效配置直连启动接口返回错误且不残留空 Job；从已保存工作流启动的运行绑定来源工作流 ID 与名称；边 ID 空/重复被校验拒绝；运行回放在画布上标出被剪枝的分支边；损坏工作流文件在列表告警中带文件名展示；存在未保存改动时离开当前文档触发确认；运行配置上限可从 Inspector 保存；多标签旧版本保存返回冲突；Chat GraphLoop 暴露步骤后停止；运行版本编辑可修复未冻结结构。既有 Graph 画布/运行用例全部回归通过。
