...
- 会话 / Job / 摘要等**元数据**是单文件 JSON，走「临时文件 + fsync + rename」原子整写。
...
└── sessions/{sessionID}/
    ...
    └── .meta/summary.json                  上下文摘要（单文件 JSON）
```

| 实体 | 路径 API（`types/path/path.go`） | 格式 | 写方式 |
|------|------|------|------|
| 聊天消息 | `MessagesFilePath`（`path.go:91`） | JSONL，一行一条 `schema.Message` | 追加为主；截断/缝合整写 |
| 上下文摘要 | `SummaryFilePath`（`path.go:96`） | 单文件 JSON | 原子整写 |
| Session 元数据 | Session `.meta/meta.json` | 单文件 JSON | 原子整写 |
| Job 元数据 | `JobMetaFile`（`path.go:351`） | 单文件 JSON | 原子整写 |
| 图运行事件 | `GraphRunEventsFile`（`path.go:290`） | JSONL | 追加，用行号做游标 |
...
```
Agent 执行器（eino / acp / shell）
  → services/agent/chatctx.ChatContextManager        编排：截断孤儿尾、追加、摘要
...
- **`summary.json` 可降级**：读失败当「无摘要」处理；`summary.index` 越界 clamp 到 `[0,len]` 并记录 Error。
...
- **summary / messages 跨文件不一致**：partial crash 导致 `summary.index > len(msgs)`，多处 clamp + Error。
