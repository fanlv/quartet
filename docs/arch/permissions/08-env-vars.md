...
| 环境变量 | 常量定义 | 默认 | 作用 |
|----------|----------|------|------|
| `X_AGENT_AUTH` | `consts.EnvKeyAgentAuth` | 未设 | HTTP API Bearer Token；逗号分隔多个；未设即"开放访问"（运行在用户个人电脑上，默认就够用）。 |
...
| `QUARTET_LISTEN_ADDR` | `consts.EnvKeyListenAddr` | 默认本机绑定 | 修改 HTTP 监听地址；要给 LAN 用得设成 `0.0.0.0:port`，**必须配合 `X_AGENT_AUTH` 一起用**。 |
...
- **`X_AGENT_AUTH` 未设** = API 完全开放给任何能访问端口的人。本机 Loopback 监听场景下没问题；任何形式的 LAN/容器/隧道暴露都必须打开。
