...
Web 与 iOS 拉取和预取的节奏一致：

- 当前 session 同步加载最新 80 条并立即渲染；若遇到 assistant/tool 组合边界，会向前扩展少量记录保证组合完整。
- 首屏完成后后台预取更早 80 条，翻页时直接消费这一页，用户不必到顶干等一次网络往返。
- 当前 session 读完后，以同样方式跨到前一个 session；Graph 只为当前选中的 session 保持分页窗口。
- 一页拉回来全是列表里已有的记录时（游标重叠、别端并发写入）不能当成「没有更早了」，要继续往前取，并给一个明确的尝试上限；连着取到的多页之间也要去重。

渲染窗口两端的实现不同，取舍见下：

- Web 用尾部对齐的窗口，每次 prepend 都必须按新增条数扩大窗口。没扩窗时整页新数据会落进窗口之外的隐藏区，而隐藏区就在列表顶部——用户刚才还在看的内容会当场从渲染里消失。
- iOS 用「跳过多少条更早历史」描述窗口，并在用户开始向上浏览时把这个跳过量固定住。尾部追加、最新页回灌的折叠、翻页 prepend 都不改它，窗口起点对应的那条消息因此始终不变。用户滚回底部才重新尾部对齐，把浏览期间扩开的渲染量收回去。

iOS 的阅读位置维持是硬约束，规则见 `ios/AGENTS.md`：只在用户真的滚到列表顶部时翻页，取回的一页由代码显式把翻页前视口顶部那条消息重新对齐回顶部；不依赖滚动容器自己保住某一行，也不用会把滚动结果写回状态的双向位置绑定。顶部的翻页触发区常驻且高度恒定。
...
- **iOS 流式输出时上滑，滚动条瞬间跳到顶部**：一串缺陷叠在一起。渲染窗口按「渲染多少条」描述且尾部对齐，流式每追加一条就从列表顶部挤掉一条；翻页提前一整页触发，锚点根本不在视口里；翻页后的位置还原改成依赖双向绑定的滚动位置，而滚动容器并不保证会替你保住某一行，于是视口上方插进来的整页内容直接把阅读位置推走；防重入的闸门在设置它的那次状态写入里就被自己清掉，一次滑动能连着翻好几页；顶部触发区是条件渲染且高度随加载状态变化，移除时收不到可见性回调、状态一直停在旧值。修复：窗口改成「跳过多少条」并在进入浏览态时固定；翻页只在真的到顶时触发，取回的一页由代码显式把翻页前视口顶部那条重新对齐回顶部；滚动一律走单向命令；闸门改由还原动作释放；触发区常驻且高度恒定。
- **iOS 每轮结束后向前翻页「转一下没反应」**：最新页回灌把翻页游标退回了最新页，于是之后每次上翻都先重新拉一遍列表里已有的页、去重后什么都没加载，翻过的页数越多要空转的次数越多。修复：同一 session 的回灌保留游标；同时让取页在「整页都是已有记录」时继续往前取。
- **iOS 历史图片撑开阅读位置**：图片加载中占固定矮占位，解码完成后按真实尺寸展开。翻页 prepend 进来的历史图片会在位置还原之后陆续撑开视口上方的内容，表现为「越滚越回不到底部」。修复：命中缓存的图片第一帧就用真实图片渲染，并记住渲染过的高度供占位预留同样空间。首次加载仍会变一次高度，要彻底消掉需要消息里带上图片尺寸。
...
| iOS 消息图片与尺寸备忘 | `ios/Quartet/Features/Chat/MessageImageViewer.swift`、`ios/Quartet/Core/Networking/ChatImageLoader.swift` |
