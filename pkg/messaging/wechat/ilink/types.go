...
// VoiceItem holds voice content. Note that WeChat may embed its own speech-
// to-text transcription in Text, which saves us from decrypting the payload
// just to extract what the user said.
...
// Session-expired and transport error codes returned in GetUpdatesResponse.
const (
	// ErrCodeSessionExpired is returned when the bot_token has expired and a
	// fresh scan-to-login is required.
