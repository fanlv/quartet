...
// cancel on Quartet so Cancel() can be called from another goroutine.
...
func (d *Quartet) Run(ctx context.Context, userMessages []*schema.Message, handler agui.EventHandler) (retErr error) {
	d.runStart()
	defer d.runEnd()

	// Reject no-op user messages early. Without this guard, an HTTP
	// request that passed handler-level validation (len(messages) > 0)
	// but whose only message had empty Content and no usable multimodal
	// parts would still reach BeginRun, persist a phantom user turn, and
	// generate an assistant reply against stale history. Mirrors the
	// "empty prompt" defence on the ACP side (services/agent/acp/agent.go)
	// so both backends agree on what counts as a real turn.
	...
	// history reload, and ACP drift detection) while the job is reported
	// as completed. Goroutine-safe via round.PersistErr because onFlush /
	// onStitch may fire from the deferred cleanup path after the main
	// loop exits.
	...
	d.builder.SetLogLabel(fmt.Sprintf("eino jobId=%s", d.jobID))

	// Token usage is recomputed by reloading the full history and
	// tokenising it. On long sessions (190k threshold for summarisation,
	// per services/agent/middlewares/summarization.go) doing this per
	// flush approaches O(history * flushCount) per Run. Debounce so the
	// UI still gets running usage but the work amortises across bursts:
	// at most one recompute per tokenUsageMinInterval, plus a final
	// guaranteed recompute at Run end (deferred below).
	var (
	...
	recomputeTokenUsage := func(force bool) {
		...
		if err != nil {
			...
			logger.Debugf(logCtx, "[eino] recompute token usage skipped: jobId=%s err=%v", d.jobID, err)
			return
		}
		if err := handler.OnTokenUsage(tokenizer.MessagesTokenCounter(logCtx, reloaded)); err != nil {
			...
			logger.Debugf(logCtx, "[eino] handler OnTokenUsage failed: jobId=%s err=%v", d.jobID, err)
		}
	}
	...
	d.builder.Reset(handler, func(msgs []*schema.Message) {
		...
		if err := ctxMgr.AppendMessages(persistCtx, msgs...); err != nil {
			logger.Warnf(logCtx, "[eino] incremental persist failed: jobId=%s err=%v", d.jobID, err)
	...
	d.builder.SetStitcher(func(toolCallID string, real *schema.Message) {
		...
		if _, err := ctxMgr.ReplacePlaceholderToolResult(persistCtx, toolCallID, real); err != nil {
			logger.Warnf(logCtx, "[eino] placeholder stitch failed: jobId=%s toolCallId=%s err=%v", d.jobID, toolCallID, err)
	...
	// returned to the job runner as a Run failure. Without this, an event.Err
	// or stream Recv error would only reach handler.OnError (UI surface) and
	// the Run would return nil — making the job runner believe the iteration
...
// reference. The Eino entry path forwards every HTTP request straight to
// Run, but earlier filtering only checked len(messages) > 0; an empty
// Content + empty UserInputMultiContent slice still passed through and
// produced a no-op assistant turn against stale history. The ACP path
// already rejects this case in its own way (extractTextFromMessages →
// "empty prompt"); this helper makes the Eino path symmetric while still
// admitting valid image-only multimodal turns whose text Content is
...
// Run's error so job runners can mark the iteration as failed; the live
// UI handler still sees OnError before the abort so the bubble closes.
func (d *Quartet) handleEvent(ctx context.Context, handler agui.EventHandler, adapter *roundAdapter, event *AgentEvent) error {
	if event.Err != nil {
