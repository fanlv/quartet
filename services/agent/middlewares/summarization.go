...
// QUARTET_SUMMARIZATION_TOKEN_THRESHOLD — e.g. 900_000 on 1M-context models,
...
const (
	...
	envSummarizationTokenThreshold     = "QUARTET_SUMMARIZATION_TOKEN_THRESHOLD"
)
...
func NewSummarizationMW(ctx context.Context, chatModel model.BaseChatModel, repo repository.ChatContextRepo) (adk.ChatModelAgentMiddleware, error) {
	...
	mw, err := summarization.New(ctx, &summarization.Config{
		...
		Finalize: func(ctx context.Context, originalMessages []adk.Message, summary adk.Message) ([]adk.Message, error) {
			...
			err := repo.SaveSummaryMessage(ctx, &repository.SummaryMessage{
				Index:   index,
				Message: summary,
			})
			if err != nil {
