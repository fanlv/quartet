...
func New(repo store.ChatContextRepo, toucher SessionToucher, sessionID string) *ChatContextManager {
...
func (m *ChatContextManager) MessagesFingerprint(ctx context.Context) (store.MessagesFingerprint, error) {
	fp, err := m.repo.MessagesFingerprint(ctx)
	if err != nil {
		return store.MessagesFingerprint{}, fmt.Errorf("compute messages fingerprint failed: %w", err)
	}
	return fp, nil
}
...
// two steps and corrupt history. The lock is owned by the store
// layer (einocli/store/session_locks.go) so every write entry-point on
...
func (m *ChatContextManager) BeginRun(ctx context.Context, userMessages ...*schema.Message) (bool, error) {
	...
	var truncated bool
	err := m.repo.WithLock(ctx, func(tx store.LockedRepo) error {
...
func (m *ChatContextManager) truncateOrphanedTailLocked(ctx context.Context, tx store.LockedRepo) (bool, error) {
