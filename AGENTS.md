...
make build-eino-cli       # Build/install eino-cli; required for Eino probe and settings support
...
make install-skill        # Install one project skill selected by SKILL_NAME
make clean                # Remove bin/
```

> `make build` 和 `make build-acp` 目前仍残留在 Makefile 中，但引用的 `cmd/acp` 已不存在，不能作为有效构建入口；使用 `make build-all` 或具体的 `build-cli`、`build-eino-cli`、`build-web` 目标。
...
- `/api/v1/health`、`POST /api/v1/auth/init`、`POST /api/v1/auth/login` 不走会话鉴权；health 只用于探测服务和认证初始化状态。
- 其余私有 `/api/v1/*` 默认走 `quartet_session` Cookie 会话鉴权并按路由校验 RBAC 权限；除 `GET`、`HEAD`、`OPTIONS` 外还必须携带登录响应返回的 `X-CSRF-Token`。
- `/api/v1/public/*` 只提供只读分享：Job 分享路由通过 `shareTokenMiddleware` 校验 `shareToken`，文件预览路由由 handler 校验独立的 `fileShareToken`。
- 当前接口风格是混合型：
...
- `cmd/quartet-cli` — quartet-cli 命令行入口，通过后端 HTTP API 登录并按 base URL 保存 Cookie/CSRF 会话，管理 graph workflow（agent 库；含手动运行）、cron 定时任务、查询 workspace/job/agent、停止 Job，以及发送微信主动消息。
- `cmd/grok-acp-demo` — Grok ACP 协议联调和事件观察用的开发入口，不参与 Web 服务运行。
...
- `ios/` — 原生 SwiftUI 客户端，提供局域网连接与登录、Job/对话、附件、Graph 运行、定时任务和统计能力；目录内还有更具体的 `ios/AGENTS.md`。

### 类型与路径

- `types/model` — Auth、Agent、Job、Session、Graph、Schedule、MessagePreset、IM、Workspace 与事件等共享 Request/Response 和领域结构体。
...
- `repository/` — 基于本地文件的存储层，负责 auth、settings、Agent catalog、jobs、sessions、Graph Workflows/Runs、schedules、message presets、prompts、workspace、分享、IM/WeChat outbox、用户输入、聊天上下文与最近目录等数据的读写，以及原子写入、损坏处理、ID 生成和并发锁。
...
- `services/auth` — 初始化、登录、Cookie/CSRF 会话、用户/角色与 RBAC 权限管理。
- `services/einocli` — 设置页 eino tab 的后端编排：exec `eino-cli models` / `eino-cli systemprompt` 子命令读写 eino-cli 自管配置（密钥不出 eino-cli 进程）。
- `services/graph` — Graph Workflow 校验、DAG/Loop 调度、变量、运行版本、恢复、控制、事件和 Hook。
- `services/job` — Job 执行核心，覆盖交互式消息运行、持久消息队列、状态恢复、观察记录和事件分发。
...
- `pkg/messaging/wechat` — 微信 IM 接入，包括 ilink 客户端、CDN、登录、媒体、回复与主动推送（`Replier.SendText`，经 `POST /api/v1/wechat/send` 暴露给定时任务/脚本，ContextToken 持久化在 `$LOCAL_MEMORY/quartet/data/wechat/accounts/user_tokens.json`）。
